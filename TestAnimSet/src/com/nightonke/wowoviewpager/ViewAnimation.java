...
public class ViewAnimation {
	...
	/**
	 * add a pageAnimation to the view the pageAnimation plays from
	 * pageAnimation.getPage()
	 * 
	 * @param pageAnimation
	 *            the new pageAnimation
	 */
	public void addPageAnimaition(PageAnimation pageAnimation) {
		int page = pageAnimation.getPage();

		if (pageAnimations == null)
			pageAnimations = new ArrayList<ArrayList<PageAnimation>>();
		while (page + 1 > pageAnimations.size()) {
			pageAnimations.add(new ArrayList<PageAnimation>());
		}

		ArrayList<PageAnimation> pageAnimationsInPage = pageAnimations
				.get(page);
		if (pageAnimationsInPage == null)
			pageAnimationsInPage = new ArrayList<PageAnimation>();
	 ...
	 * 
	 * @param page
	 *            the starting page, the corresponding animations will end at
	 *            page + 1
	 * @param positionOffset
	 *            the positionOffset of viewpager
	 */
	public void play(int page, float positionOffset) {
		if (page >= pageAnimations.size())
			return;
		ArrayList<PageAnimation> pageAnimationsInPage = pageAnimations
				.get(page);
		if (pageAnimationsInPage != null) {
	 ...
	 * 
	 * @param page
	 *            the end position of the page
	 */
	public void end(int page) {
		if (page >= pageAnimations.size() || page < 0)
			return;
		ArrayList<PageAnimation> pageAnimationsInPage = pageAnimations
				.get(page);
		if (pageAnimationsInPage != null) {
